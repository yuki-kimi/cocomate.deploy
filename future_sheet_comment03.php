...
// あなたのコメントを取得
$query = "SELECT family_comment_03 FROM profile_familyvalue_table WHERE user_id = ?";
...
if ($other_user_id) {
    $query = "SELECT i.first_name, p.family_comment_03
    ...
    if ($other_user_data) {
        ...
        $other_user_family_comment = $other_user_data['family_comment_03'] ?? 'コメントなし';
    }
}
?>
...
<h2 class="section-title">３．どのような生活をしていきたいのか。</h2>
...
<div class="button-section">
    ...
    <button class="nav-button" onclick="window.location.href='future_sheet_comment02.php?user_id=<?php echo htmlspecialchars($other_user_id, ENT_QUOTES, 'UTF-8'); ?>'">前にもどる</button>    
    <!-- 次にすすむボタン -->
    <button class="nav-button" onclick="window.location.href='future_sheet_comment04.php?user_id=<?php echo htmlspecialchars($other_user_id, ENT_QUOTES, 'UTF-8'); ?>'">次にすすむ</button>
</div>
