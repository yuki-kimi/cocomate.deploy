...
<h2 class="section-title">６．家族や友人との付き合い方について（周囲への紹介の仕方、家族旅行や冠婚葬祭など）</h2>
...
// あなたのコメントを取得
$query = "SELECT family_comment_06 FROM profile_familyvalue_table WHERE user_id = ?";
...
if ($other_user_id) {
    $query = "SELECT i.first_name, p.family_comment_06
    ...
    if ($other_user_data) {
        ...
        $other_user_family_comment = $other_user_data['family_comment_06'] ?? 'コメントなし';
    }
}
?>
...
<div class="advice-box">
    <div class="advice-text-container">
        ...
        <p>人との距離感は生き方に影響する大事な要素。家族や友達との関係性を聞いてみると、その人の心のつながり方が見えてきます。</p>
        <p>自分のスタンスも添えながら話すと、対話が柔らかくなります。違いを比べるというより、理解し合う気持ちが大切です。</p>
        </div>
    </div>
...
<div class="button-section">
    ...
    <button class="nav-button" onclick="window.location.href='future_sheet_comment05.php?user_id=<?php echo htmlspecialchars($other_user_id, ENT_QUOTES, 'UTF-8'); ?>'">前にもどる</button>    
    <!-- 次にすすむボタン -->
    <button class="nav-button" onclick="window.location.href='future_sheet_comment07.php?user_id=<?php echo htmlspecialchars($other_user_id, ENT_QUOTES, 'UTF-8'); ?>'">次にすすむ</button>
</div>
