...
<h2 class="section-title">１０．子どもを授かりたいか。（妊活を希望するかなど）</h2>
...
// あなたのコメントを取得
$query = "SELECT family_comment_10 FROM profile_familyvalue_table WHERE user_id = ?";
...
if ($other_user_id) {
    $query = "SELECT i.first_name, p.family_comment_10
    ...
    if ($other_user_data) {
        ...
        $other_user_family_comment = $other_user_data['family_comment_10'] ?? 'コメントなし';
    }
}
?>
...
<div class="advice-box">
    <div class="advice-text-container">
        ...
        <p>正解のないテーマなので、安心して話せる空気が大切です。</p>
        <p>「今はどう思ってる？」や「将来どうなるかはわからないけど…」と柔らかく話すことで、プレッシャーなくお互いの価値観を共有できます。</p>
        <p>焦らず、今の気持ちを伝えるだけで十分です。</p>
        </div>
    </div>
...
<div class="button-section">
    ...
    <button class="nav-button" onclick="window.location.href='future_sheet_comment09.php?user_id=<?php echo htmlspecialchars($other_user_id, ENT_QUOTES, 'UTF-8'); ?>'">前にもどる</button>    
    <!-- 次にすすむボタン -->
    <button class="nav-button" onclick="window.location.href='future_sheet_comment_end.php?user_id=<?php echo htmlspecialchars($other_user_id, ENT_QUOTES, 'UTF-8'); ?>'">次にすすむ</button>
</div>
