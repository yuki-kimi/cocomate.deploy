...
<h2 class="section-title">９．生活について（同居or別居、食事、家事、休暇、習慣など）</h2>
...
// あなたのコメントを取得
$query = "SELECT family_comment_09 FROM profile_familyvalue_table WHERE user_id = ?";
...
if ($other_user_id) {
    $query = "SELECT i.first_name, p.family_comment_09
    ...
    if ($other_user_data) {
        ...
        $other_user_family_comment = $other_user_data['family_comment_09'] ?? 'コメントなし';
    }
}
?>
...
<div class="advice-box">
    <div class="advice-text-container">
        ...
        <p>生活リズムや家での過ごし方など、実はとても相性が出る部分。「朝は強い？夜型？」など、ちょっとした質問から広がります。</p>
        <p>自分のペースを伝えながら、相手の心地よさも大切にすることで、ふたりで作る日常がイメージしやすくなります。</p>
        </div>
    </div>
...
<div class="button-section">
    ...
    <button class="nav-button" onclick="window.location.href='future_sheet_comment08.php?user_id=<?php echo htmlspecialchars($other_user_id, ENT_QUOTES, 'UTF-8'); ?>'">前にもどる</button>    
    <!-- 次にすすむボタン -->
    <button class="nav-button" onclick="window.location.href='future_sheet_comment10.php?user_id=<?php echo htmlspecialchars($other_user_id, ENT_QUOTES, 'UTF-8'); ?>'">次にすすむ</button>
</div>
