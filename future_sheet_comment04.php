...
<h2 class="section-title">４．自身のこだわりやゆずれないこと</h2>
...
// あなたのコメントを取得
$query = "SELECT family_comment_04 FROM profile_familyvalue_table WHERE user_id = ?";
$stmt = $pdo->prepare($query);
$stmt->execute([$user_id]);
$user_family_comment = $stmt->fetchColumn();

// 他のユーザーのIDを取得（URLパラメータが必須）
$other_user_id = $_GET['user_id'] ?? null;  // URLパラメータがない場合、nullになる

if (!$other_user_id) {
    echo "他のユーザーIDが指定されていません。";
    // リダイレクト処理を加えてみる
    header("Location: error_page.php"); // エラーページにリダイレクト
    exit;
}
...
if ($other_user_id) {
    $query = "SELECT i.first_name, p.family_comment_05
    ...
    if ($other_user_data) {
        ...
        $other_user_family_comment = $other_user_data['family_comment_05'] ?? 'コメントなし';
    }
}
?>
...
<div class="advice-box">
    <div class="advice-text-container">
        ...
        <p>こだわりにはその人の個性がよく表れます。「どうしてそれを大事にしてるの？」とやさしく聞いてみると、その人らしさがぐっと見えてきます。</p>
        <p>相手のこだわりを尊重しつつ、自分の譲れない部分も軽くシェアするとバランスがとりやすいです。</p>
        </div>
    </div>
...
<div class="button-section">
    ...
    <button class="nav-button" onclick="window.location.href='future_sheet_comment03.php?user_id=<?php echo htmlspecialchars($other_user_id, ENT_QUOTES, 'UTF-8'); ?>'">前にもどる</button>    
    <!-- 次にすすむボタン -->
    <button class="nav-button" onclick="window.location.href='future_sheet_comment05.php?user_id=<?php echo htmlspecialchars($other_user_id, ENT_QUOTES, 'UTF-8'); ?>'">次にすすむ</button>
</div>
</body>
</html>
