...
<h2 class="section-title">７．理想のパートナーシップ像について</h2>
...
// あなたのコメントを取得
$query = "SELECT family_comment_07 FROM profile_familyvalue_table WHERE user_id = ?";
...
if ($other_user_id) {
    $query = "SELECT i.first_name, p.family_comment_07
    ...
    if ($other_user_data) {
        ...
        $other_user_family_comment = $other_user_data['family_comment_07'] ?? 'コメントなし';
    }
}
?>
...
<div class="advice-box">
    <div class="advice-text-container">
        ...
        <p>近すぎず遠すぎず、ちょうどいい距離感は人それぞれ。「どれくらいの頻度で会いたい？」や「一人の時間ってどんな風に過ごす？」など、具体的な過ごし方を話すと自然に相性が見えてきます。</p>
        <p>心地よさを大事にしながら会話を楽しんでください。</p>
        </div>
    </div>
...
<div class="button-section">
    ...
    <button class="nav-button" onclick="window.location.href='future_sheet_comment06.php?user_id=<?php echo htmlspecialchars($other_user_id, ENT_QUOTES, 'UTF-8'); ?>'">前にもどる</button>    
    <!-- 次にすすむボタン -->
    <button class="nav-button" onclick="window.location.href='future_sheet_comment08.php?user_id=<?php echo htmlspecialchars($other_user_id, ENT_QUOTES, 'UTF-8'); ?>'">次にすすむ</button>
</div>
